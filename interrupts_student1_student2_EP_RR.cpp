...
//returns true if there exists a ready process whose priority > given priority
static bool exists_higher_priority_ready(const std::vector<PCB> &ready_queue, int priority) {
...
std::tuple<std::string> run_simulation(std::vector<PCB> list_processes) {
    ...
    unsigned int running_time = 0; // time running in current slice
    ...
    while (true) {
        ...
        //try to allocate memory and move to ready
        ...
        //if running exists and some ready has higher priority then preempt running 
        if (running.PID != -1 && running.state == RUNNING) {
            if (exists_higher_priority_ready(ready_queue, running.priority)) {
                // preempt running at end of this tick and requeue it
                states old_state = RUNNING;
                running.state = READY;
                sync_queue(job_list, running);
                execution_status += print_exec_status(current_time + 1, running.PID, old_state, READY);

                //requeue preempted process at end of ready queue
        ...
        if ((running.PID == -1) || (running.state != RUNNING)) {
            if (!ready_queue.empty()) {
                // select highest priority index
                int idx = pick_highest_priority_index(ready_queue);
                if (idx >= 0) {
                    PCB proc = ready_queue[idx];
                    ready_queue.erase(ready_queue.begin() + idx);

                    states old_state = proc.state;
                    proc.state = RUNNING;
                    if (proc.start_time == -1) proc.start_time = current_time;
                    sync_queue(job_list, proc);

                    // log transition
                    execution_status += print_exec_status(current_time, proc.PID, old_state, RUNNING);

                    // print memory snapshot only the first time PID starts running
                    if (snapshot_done.count(proc.PID) == 0) {
                        snapshot_done.insert(proc.PID);
                    ...
                    // put into running
                    running = proc;
                    running_time = 0; // reset quantum counter for this run burst
                }
            }
        }
